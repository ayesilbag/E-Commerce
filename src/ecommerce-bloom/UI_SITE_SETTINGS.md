...
## Seed (bizdenalbizdensat / PROD)

`bizdenalbizdensat` kodlu kayıt için **yalnızca yasal form içerikleri** uygulama başlangıcında seed edilir ([bizdenalbizdensat.com](https://www.bizdenalbizdensat.com/)):

| Dosya | Açıklama |
|-------|----------|
| `src/Infrastructure/Data/Seeding/BizdenalbizdensatLegalFormsSeeder.cs` | 5 yasal sayfa içeriğini yazar |
| `src/Infrastructure/Data/Seeding/BizdenalbizdensatLegalPageContent.cs` | HTML metinler (ön bilgilendirme dahil) |

Storefront `.env`: `VITE_UI_CODE=bizdenalbizdensat`

Sabit sayfa slug’ları: `/hakkimizda`, `/on-bilgilendirme-formu`, `/teslimat-ve-iade`, `/gizlilik`, `/mesafeli-satis`

- **Development:** `TrySeedAsync` içinde çalışır.
- **Production:** `SeedBizdenalbizdensatLegalFormsAsync()` — marka/iletişim alanlarına dokunmaz; kayıt yoksa minimal kabuk oluşturur.

iyzico logosu admin’den yüklenir (Visa/Mastercard upload kaldırıldı).
...
3. Marka, iletişim ve **ödeme kriterleri** (sayfa URL'leri, SSL, logolar) → Kaydet
4. Storefront `.env`:
   ```env
   VITE_UI_CODE=bizdenal
   VITE_API_BASE_URL=https://api.example.com/api
